  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Sambutin.id — Undangan Digital Pernikahan Premium" },
      {
        ...
        content: "Mengabadikan kisah cinta dalam undangan digital yang berkesan.",
      },
...
function HeroSceneDecor() {
  ...
  return (
    <div aria-hidden className="pointer-events-none absolute inset-0 z-0 overflow-hidden">
      ...
      <div
        className="absolute top-[70%] right-[20%] w-32 h-32 doodle-circle spin-slow opacity-40"
...
function SectionDecor({
  ...
  variant: "keunggulan" | "fitur" | "portfolio" | "process" | "testimoni" | "faq" | "kontak";
}) {
        ...
        <div className="flex gap-8 whitespace-nowrap text-primary-foreground/70 font-script text-xl animate-drift">
          {Array.from({ length: 8 }).map((_, i) => (
            <span key={i} className="flex items-center gap-8">
              <span>sambut.in</span> <DiamondTiny className="w-3 h-3 text-primary-foreground/70" />{" "}
              <span>sambut.in</span> <DiamondTiny className="w-3 h-3 text-primary-foreground/70" />{" "}
              <span>sambut.in</span> <DiamondTiny className="w-3 h-3 text-primary-foreground/70" />
            </span>
          ))}
...
function Navbar({ content }: { content: SiteContent }) {
  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-40 transition-all duration-500 ${scrolled ? "bg-background/85 backdrop-blur-md border-b border-border/60 py-3" : "bg-transparent py-5"}`}
    >
        ...
        <div className="lg:hidden bg-background/95 backdrop-blur-md border-t border-border mt-3 px-5 py-5 space-y-3 fade-up">
          {content.nav.map((n) => (
            <a
              ...
              className="block py-2 font-serif text-lg text-foreground/80"
            >
              {n.label}
            </a>
          ))}
  ...
  return (
    <section className="relative pt-32 sm:pt-40 pb-20 sm:pb-28 overflow-hidden">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <RoseSilhouette
          className="absolute top-[18%] left-[42%] w-10 h-10 text-primary/40 wiggle"
          style={{ ["--r" as any]: "-12deg" }}
        />
        <StarBurst
          className="absolute top-[8%] right-[28%] w-8 h-8 text-gold drift"
          style={{ ["--r" as any]: "10deg" }}
        />
        <RingsDuo
          className="absolute bottom-[18%] left-[34%] w-12 h-8 text-primary/45 drift"
          style={{ ["--r" as any]: "8deg", animationDelay: ".8s" }}
        />
      </div>
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8 grid gap-12 lg:gap-16 items-center text-center">
        ...
        <div className="hidden">
          ...
          <div
            className="absolute -top-6 -left-4 sm:-left-10 w-36 sm:w-44 polaroid -rotate-6 float-soft"
            style={{ ["--r" as any]: "-6deg" }}
          >
            <div className="aspect-square rounded-sm bg-secondary grid place-items-center text-primary/70">
              <RingsDuo className="w-20 h-14" />
            </div>
            <p className="font-script text-center text-lg text-primary mt-2">forever</p>
          </div>
          <div
            className="absolute bottom-6 -right-2 sm:-right-8 w-40 sm:w-48 polaroid rotate-6 float-soft"
            style={{ ["--r" as any]: "6deg", animationDelay: "1.2s" }}
          >
            <div className="aspect-square rounded-sm bg-accent grid place-items-center text-primary/70">
              <BotanicalSpray className="w-20 h-20" />
            </div>
          ...
          <div
            className="absolute -bottom-4 left-6 sticker h-14 w-14 wiggle text-primary"
            style={{ ["--r" as any]: "-10deg" }}
          >
            <WaxSeal className="w-9 h-9" />
          </div>
        </div>
      </div>
    </section>
        ...
        <div className="mt-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-6">
          {k.items.map((it, i) => {
            ...
            return (
              <article
                key={i}
                className={`paper-card p-7 ${meta.span ?? ""} ${meta.tint} relative transition-all duration-500 hover:-translate-y-1 hover:shadow-[0_24px_50px_-20px_oklch(0.36_0.13_18/0.35)]`}
              >
                <div
                  className={`h-12 w-12 rounded-xl grid place-items-center ${isDark ? "bg-gold/30" : "bg-background"} mb-5`}
                >
                  <Icon size={22} className={isDark ? "text-primary-foreground" : "text-primary"} />
                </div>
                <h3 className="font-serif text-2xl mb-2">{it.title}</h3>
                <p
                  className={`text-sm leading-relaxed ${isDark ? "text-primary-foreground/85" : "text-muted-foreground"}`}
                >
                  {it.desc}
                </p>
              </article>
        ...
        <div className="mt-16 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
          {f.items.map((it, i) => {
            ...
            return (
              <div
                key={i}
                className="paper-card bg-card p-8 relative transition-transform duration-500 hover:rotate-0 hover:-translate-y-2"
              >
...
/* ---------------- PORTFOLIO ---------------- */
/* ---------------- PORTFOLIO ---------------- */
function PortfolioBadge({ label }: { label: string }) {
  const lower = label.toLowerCase().trim();

  // Palette variants — each maps to a distinct on-brand accent color.
  // "populer" keeps the original primary (rose/terracotta) look.
  // "new"  → sage green, "hot" → warm ember, "best" → deep gold.
  type Variant = { style: "solid" | "outline"; base: string; textOnSolid?: string };
  const variants: Record<string, Variant> = {
    populer: { style: "solid", base: "var(--primary)", textOnSolid: "var(--primary-foreground)" },
    pop: { style: "solid", base: "var(--primary)", textOnSolid: "var(--primary-foreground)" },
    hot: { style: "solid", base: "oklch(0.62 0.19 35)", textOnSolid: "oklch(0.98 0.02 80)" }, // ember / warm orange-red
    best: { style: "solid", base: "oklch(0.62 0.13 75)", textOnSolid: "oklch(0.98 0.02 90)" }, // deep gold
    favorit: { style: "solid", base: "oklch(0.62 0.19 35)", textOnSolid: "oklch(0.98 0.02 80)" },
    new: { style: "solid", base: "oklch(0.55 0.11 155)", textOnSolid: "white" }, // sage green
  };
  const key = Object.keys(variants).find((k) => lower.startsWith(k)) ?? "";
  const v: Variant = variants[key] ?? { style: "outline", base: "var(--primary)" };
    ...
    <div className="absolute top-3 right-3 z-10 pointer-events-none">
      {v.style === "solid" ? (
        <span
          ...
          style={{
            background: `linear-gradient(135deg, ${v.base}, color-mix(in oklab, ${v.base} 70%, black))`,
            color: v.textOnSolid,
            boxShadow: `inset 0 0 0 1px color-mix(in oklab, white 30%, transparent), 0 8px 20px -8px color-mix(in oklab, ${v.base} 60%, black)`,
          }}
        >
          {label}
        </span>
        ...
        <span
          className="inline-flex items-center rounded-full bg-card/95 backdrop-blur-sm px-3.5 py-1 text-[10px] font-semibold uppercase tracking-[0.22em]"
          style={{
            color: v.base,
            boxShadow: `inset 0 0 0 1px color-mix(in oklab, ${v.base} 45%, transparent), 0 6px 16px -8px color-mix(in oklab, ${v.base} 40%, black)`,
          }}
        >
          {label}
        </span>
...
function Portfolio({ content }: { content: SiteContent }) {
  ...
  const filtered =
    active === (p.cats[0] ?? "Semua") ? items : items.filter((i) => i.cat === active);
  const PER_PAGE = 6;
  ...
  const go = (dir: number) => {
    ...
    setPage((currentPage + dir + totalPages) % totalPages);
  };
  const goTo = (i: number) => {
        ...
        <div className="mt-10 flex flex-wrap justify-center gap-2.5">
          {p.cats.map((c) => (
            <button
              ...
              className={`rounded-full px-5 py-2 text-xs font-semibold tracking-wide transition-all ${active === c ? "bg-primary text-primary-foreground" : "bg-card text-foreground/70 hover:bg-secondary/60"}`}
            >
              {c}
            </button>
          ))}
        ...
        <div className="relative mt-12">
          {totalPages > 1 && (
            <>
              <button
                ...
                className="hidden md:flex absolute -left-4 lg:-left-6 top-1/2 -translate-y-1/2 z-10 h-11 w-11 items-center justify-center rounded-full bg-card border border-primary/25 text-primary shadow-[0_10px_24px_-12px_rgba(0,0,0,0.3)] hover:bg-primary hover:text-primary-foreground transition-colors"
              >
                ‹
              </button>
              <button
                ...
                className="hidden md:flex absolute -right-4 lg:-right-6 top-1/2 -translate-y-1/2 z-10 h-11 w-11 items-center justify-center rounded-full bg-card border border-primary/25 text-primary shadow-[0_10px_24px_-12px_rgba(0,0,0,0.3)] hover:bg-primary hover:text-primary-foreground transition-colors"
              >
                ›
              </button>
            </>
          ...
          <div className="overflow-hidden">
            <div
              ...
              className="grid grid-cols-2 lg:grid-cols-3 gap-5 lg:gap-8 portfolio-slide"
              style={{ ["--slide-from" as any]: direction === 1 ? "60px" : "-60px" }}
            >
              {pageItems.map((it, i) => (
                <figure
                  key={`${currentPage}-${i}`}
                  className="group relative paper-card overflow-hidden bg-card p-3 sm:p-4 portfolio-card-in"
                  style={{ animationDelay: `${i * 70}ms` }}
                >
                  <div className="relative overflow-hidden rounded-md bg-secondary/30 aspect-[3/4]">
                    <img
                      src={(it as any).image || it.src}
                      alt={`Undangan ${it.title}`}
                      loading="lazy"
                      className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105"
                    />
                    <div className="absolute inset-0 bg-primary/0 group-hover:bg-primary/20 transition-colors" />
                    {it.price && (
                      <div className="absolute bottom-3 left-3 flex items-center gap-2 bg-primary backdrop-blur-sm border border-primary/25 text-white px-3 py-1.5 rounded-full shadow-[0_6px_14px_-6px_rgba(0,0,0,0.25)]">
                        {it.priceOriginal && (
                          <span className="text-[10px] font-medium text-white line-through tracking-wide">
                            {it.priceOriginal}
                          </span>
                        )}
                        <span className="text-[11px] font-bold tracking-wide">{it.price}</span>
                      </div>
                    )}
          ...
          {totalPages > 1 && (
            <div className="mt-8 flex items-center justify-center gap-3">
              <button
                onClick={() => go(-1)}
                aria-label="Sebelumnya"
                className="md:hidden h-9 w-9 flex items-center justify-center rounded-full bg-card border border-primary/25 text-primary"
              >
                ‹
              </button>
              <div className="flex items-center gap-2">
                {Array.from({ length: totalPages }).map((_, i) => (
                  <button
                    ...
                    className={`transition-all rounded-full ${i === currentPage ? "w-6 h-2 bg-primary" : "w-2 h-2 bg-primary/25 hover:bg-primary/50"}`}
                  />
                ))}
              </div>
              <button
  ...
  return (
    <section id="addon" className="relative py-24 sm:py-32 bg-[oklch(0.78_0.05_25)]">
      ...
      <div className="relative mx-auto max-w-3xl px-5 sm:px-8">
        <SectionHead eyebrow={a.eyebrow} title={renderTitle(a.title)} subtitle={a.subtitle} />
        ...
        <div className="mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-4 relative">
          ...
          {p.steps.map((s) => (
            <div key={s.n} className="relative text-center">
              <div className="mx-auto h-24 w-24 rounded-full bg-background grid place-items-center font-serif text-3xl text-primary ring-1 ring-primary/20 shadow-[var(--shadow-soft)] relative z-10">
                {s.n}
              </div>
...
function TestiCard({
  ...
  it: { text: string; name: string; meta: string; image?: string };
  i: number;
  decorIndex: number;
}) {
  ...
  return (
    <article
      className="polaroid transition-transform duration-500 hover:-translate-y-2 hover:rotate-0"
    ...
    >
      {it.image ? (
        ...
        <div
          className={`aspect-[4/3] grid place-items-center rounded-sm ${d.color} text-primary/70`}
        >
      ...
      <div className="pt-4">
        ...
        <p className="font-serif italic text-[15px] leading-snug text-foreground/85">
          &ldquo;{it.text}&rdquo;
        </p>
...
function Testimoni({ content }: { content: SiteContent }) {
  ...
  const visible = isCarousel ? items.slice(page * pageSize, page * pageSize + pageSize) : items;
  return (
    <section id="testimoni" className="relative py-24 sm:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-8">
        ...
        <div key={page} className="mt-16 grid grid-cols-2 lg:grid-cols-4 gap-5 lg:gap-8 fade-up">
          {visible.map((it, i) => (
            <TestiCard
              ...
              it={it}
              i={i}
              decorIndex={isCarousel ? page * pageSize + i : i}
            />
          ))}
        </div>
        {isCarousel && (
        ...
        <div className="mt-16 flex flex-wrap justify-center gap-3">
          {t.stats.map((s) => {
            const Icon = (Icons[s.icon as keyof typeof Icons] as LucideIcon) ?? Icons.Star;
                      ...
                      {Array.isArray(it.a) ? (
                        ...
                        <div className="px-5 sm:px-6 pb-6 text-sm text-muted-foreground leading-relaxed">
                          {it.a.type === "list" ? (
                            ...
                            <p className="whitespace-pre-line">{it.a.content}</p>
                          )}
                        </div>
                      )}
                    </div>
      ...
      <div className="mx-auto max-w-5xl px-5 sm:px-8">
        <div className="relative paper-card overflow-hidden bg-primary text-primary-foreground px-8 sm:px-16 py-16 sm:py-24 text-center">
          <div
            ...
            style={{
              backgroundImage:
                "radial-gradient(circle at 20% 20%, oklch(0.78 0.09 80/0.6), transparent 40%), radial-gradient(circle at 80% 80%, oklch(0.88 0.045 20/0.5), transparent 45%)",
            }}
          />
          <div className="absolute top-6 left-6 flex items-center gap-2 font-script text-3xl text-gold/80 rotate-[-6deg] pointer-events-none">
            with love <RoseSilhouette className="w-7 h-7" />
          </div>
          ...
          <h2 className="mt-3 font-serif text-4xl sm:text-6xl leading-tight">
            {c.title.pre}
            {c.title.em && <em className="italic text-gold">{c.title.em}</em>}
            {c.title.post}
          </h2>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="mt-10 inline-flex items-center justify-center gap-2 rounded-full px-8 py-4 bg-background text-primary font-semibold hover:bg-gold hover:text-primary transition-all"
          >
      ...
      <div className="mx-auto max-w-6xl px-5 sm:px-8 grid lg:grid-cols-2 gap-12 items-center">
        <div>
          <SectionHead
            align="left"
            eyebrow={k.eyebrow}
            title={renderTitle(k.title)}
            subtitle={k.subtitle}
          />
          <ul className="mt-8 space-y-4 text-sm">
            <li className="flex items-center gap-3">
              <Phone size={18} className="text-primary" /> {k.phone}
            </li>
            <li className="flex items-center gap-3">
              <MessageCircle size={18} className="text-primary" /> {k.waNote}
            </li>
            <li className="flex items-center gap-3">
              <MapPin size={18} className="text-primary" /> {k.location}
            </li>
          </ul>
        </div>
        <form onSubmit={onSubmit} className="paper-card bg-card p-7 sm:p-10 space-y-5">
          ...
          <Field label="Pesan">
            <textarea
              ...
              placeholder="Ceritakan kebutuhan undangan Anda..."
            />
          </Field>
  ...
  return (
    <footer className="relative pt-20 pb-10 border-t border-border/60 bg-card">
      <div className="mx-auto max-w-7xl px-5 sm:px-8 grid md:grid-cols-3 gap-12">
          ...
          <div className="mt-5 flex gap-3">
            {[
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="h-10 w-10 rounded-full grid place-items-center bg-background hover:bg-primary hover:text-primary-foreground border border-border transition-colors"
              >
                <Icon size={16} />
              </a>
            ))}
        ...
        <div>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="btn-primary mt-5 !text-xs !px-5 !py-2.5"
          >
      ...
      <div className="mt-14 pt-6 border-t border-border/60 text-center text-xs text-muted-foreground">
        © {new Date().getFullYear()} {content.branding.siteName} · Dibuat dengan{" "}
        <Heart size={12} className="inline text-primary" /> untuk pasangan istimewa.
      </div>
    </footer>
  );
}
...
function SectionHead({
  ...
  return (
    <div className={`${a} max-w-3xl`}>
      ...
      <h2 className="mt-2 font-serif text-4xl sm:text-5xl lg:text-[3.5rem] leading-[1.1] tracking-tight">
        {title}
      </h2>
      {subtitle && (
        <p className="mt-5 text-base text-muted-foreground leading-relaxed">{subtitle}</p>
      )}
    </div>
  );
}
