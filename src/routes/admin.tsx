...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin Panel — Sambutin.id" }, { name: "robots", content: "noindex,nofollow" }],
  }),
  ...
  return (
    <div className="min-h-screen bg-secondary/20">
      ...
      <header className="sticky top-0 z-30 bg-card/90 backdrop-blur-md border-b border-border">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 py-3 flex items-center gap-3 flex-wrap">
          ...
          <div>
            ...
            <p className="text-[11px] text-muted-foreground">
              {userEmail ? `Login sebagai ${userEmail}` : "Edit konten website."}
            </p>
          </div>
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            <button onClick={reset} className="btn-ghost text-destructive">
        ...
        <button onClick={logout} className="btn-ghost text-destructive" title="Keluar">
            ...
            <button
              ...
              className={`shrink-0 px-3.5 py-1.5 rounded-full text-xs font-medium transition-colors ${
                tab === t.key
                  ...
                  : "bg-background text-foreground/70 hover:bg-secondary/60"
              }`}
            >
              {t.label}
            </button>
      ...
      <main className="mx-auto max-w-4xl px-4 sm:px-6 py-8">
        ...
        {tab === "keunggulan" && (
          <ListSectionTab
            ...
            max={7}
            note="Item ke-1 dan ke-7 menampilkan layout besar. Item ke-7 berlatar gelap."
          />
        )}
        {tab === "fitur" && (
          <ListSectionTab
            ...
            max={6}
            note="Ikon mengikuti urutan: Tamu, Musik, Hati, Hadiah, Video, Kalender."
          />
        )}
        ...
        {tab === "akun" && <AkunTab email={userEmail} />}
      </main>
...
function TitleEditor({
  ...
  label: string;
  value: TitleParts;
  onChange: (v: TitleParts) => void;
}) {
  return (
    <div>
      <span className="field-label">{label}</span>
      <p className="text-[11px] text-muted-foreground -mt-1 mb-2">
        Bagian "Highlight" akan tampil{" "}
        <em className="text-primary not-italic font-semibold italic">miring & berwarna</em>.
      </p>
      <div className="grid-3">
        <input
          ...
          value={value.pre}
          onChange={(e) => onChange({ ...value, pre: e.target.value })}
        />
        <input
          className="field-input"
          placeholder="Highlight"
          value={value.em}
          onChange={(e) => onChange({ ...value, em: e.target.value })}
        />
        <input
          className="field-input"
          placeholder="Teks sesudah"
          value={value.post}
          onChange={(e) => onChange({ ...value, post: e.target.value })}
        />
      </div>
    </div>
  );
  ...
  return (
    <div>
      ...
      <div className="flex items-start gap-3">
        <div
          className="rounded-md border border-border bg-secondary/30 overflow-hidden shrink-0"
          style={{ width: 110, aspectRatio: aspect }}
        >
          {value ? (
        ...
        <div className="flex flex-col gap-2">
          ...
          <p className="text-[10px] text-muted-foreground max-w-[180px]">
            JPG/PNG, otomatis dikompres ≤900px.
          </p>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="card">
      ...
      <div className="grid-2">
        ...
        <TextField
          label="Nomor WhatsApp (format internasional, contoh: 6281234567890)"
          value={b.waNumber}
          onChange={(v) => set({ waNumber: v.replace(/[^\d]/g, "") })}
        />
      </div>
      <div className="mt-3">
        <TextArea
  ...
  return (
    <>
      <div className="card">
        ...
        <TextField
          label="Teks Script (di atas judul)"
          value={h.script}
          onChange={(v) => set({ script: v })}
        />
        <div className="mt-3">
          <TitleEditor label="Judul" value={h.title} onChange={(v) => set({ title: v })} />
        </div>
        <div className="mt-3">
          <TextArea label="Subtitle" value={h.subtitle} onChange={(v) => set({ subtitle: v })} />
        </div>
        <div className="grid-2 mt-3">
          <TextField
      ...
      <div className="card">
        ...
        <StringList
          ...
          placeholder="Contoh: RSVP Online"
        />
      </div>
    </>
  );
...
function SectionHeaderEditor<
  K extends "keunggulan" | "fitur" | "portfolio" | "process" | "testimoni" | "faq" | "kontak",
>({ section, draft, update }: { section: K; draft: SiteContent; update: UpdateFn }) {
  ...
  return (
    <div className="card">
      ...
      <div className="mt-3">
        <TitleEditor label="Judul" value={s.title} onChange={(v) => set({ title: v })} />
      </div>
    </div>
  );
}
        ...
        {s.items.map((it: any, i: number) => (
          <div key={i} className="item-card">
            ...
            <div className="flex justify-end mb-1">
              <button
                className="icon-btn"
                onClick={() => set({ items: s.items.filter((_: any, j: number) => j !== i) })}
                aria-label="Hapus"
              >
                <Trash2 size={14} />
              </button>
    ...
    <>
      ...
      <div className="card">
        ...
        <StringList
          ...
          placeholder="Contoh: Luxury"
        />
      </div>
      <div className="card">
        <h2 className="card-title">Item Portfolio</h2>
        <p className="card-sub">
          Upload gambar thumbnail untuk tiap undangan. Jika kosong, akan pakai gambar default.
        </p>
        {p.items.map((it, i) => (
          <div key={i} className="item-card">
            <span className="item-tag">#{i + 1}</span>
            <div className="flex justify-end mb-1">
              <button
                className="icon-btn"
                onClick={() => set({ items: p.items.filter((_, j) => j !== i) })}
              >
                <Trash2 size={14} />
              </button>
            </div>
            ...
            <div className="grid-2 mt-3">
              ...
              <TextField
                ...
                value={it.priceOriginal ?? ""}
                onChange={(v) => {
                  const n = [...p.items];
                  n[i] = { ...n[i], priceOriginal: v };
                  set({ items: n });
                }}
              />
            </div>
            <div className="mt-3">
              <label className="field-label">Label / Badge</label>
              <div className="flex flex-wrap gap-2 mb-2">
                {["", "New", "Populer", "Hot", "Best"].map((opt) => {
                  const selected = (it.badge ?? "") === opt;
                  return (
                    <button
                      key={opt || "none"}
                      type="button"
                      ...
                      className={`px-3 py-1 rounded-full text-xs font-semibold border transition-colors ${selected ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:bg-secondary/50"}`}
                    >
                      {opt || "Tidak ada"}
                    </button>
                  );
              ...
              <input
                className="field-input"
                placeholder="Atau ketik label custom (opsional)"
                value={it.badge ?? ""}
                onChange={(e) => {
                  const n = [...p.items];
                  n[i] = { ...n[i], badge: e.target.value };
                  set({ items: n });
                }}
              />
            </div>
        ...
        <button
          className="add-btn"
          onClick={() =>
            set({ items: [...p.items, { title: "", cat: p.cats[1] ?? "", link: "", price: "" }] })
          }
        >
          <Plus size={14} /> Tambah Item
        </button>
      ...
      <div className="card">
        ...
        <div className="grid-2">
          <TextField
            label="Label Tiket"
            value={a.ticketLabel}
            onChange={(v) => set({ ticketLabel: v })}
          />
        </div>
        <div className="mt-3">
          <TextArea label="Catatan" value={a.note} onChange={(v) => set({ note: v })} rows={2} />
        </div>
      </div>
      <div className="card">
        ...
        {a.items.map((it, i) => (
          <div key={i} className="grid grid-cols-[1fr_140px_auto] gap-2 mb-2 items-end">
            ...
            <button
              className="icon-btn"
              onClick={() => set({ items: a.items.filter((_, j) => j !== i) })}
            >
              <Trash2 size={14} />
            </button>
          </div>
        ...
        <button
          className="add-btn"
          onClick={() => set({ items: [...a.items, { name: "", price: "" }] })}
        >
          <Plus size={14} /> Tambah
        </button>
      </div>
        ...
        {p.steps.map((s, i) => (
          <div key={i} className="item-card">
            ...
            <div className="flex justify-end mb-1">
              <button
                className="icon-btn"
                onClick={() => set({ steps: p.steps.filter((_, j) => j !== i) })}
              >
                <Trash2 size={14} />
              </button>
            </div>
        ...
        <button
          ...
          onClick={() =>
            set({
              steps: [
                ...p.steps,
                { n: String(p.steps.length + 1).padStart(2, "0"), title: "", desc: "" },
              ],
            })
          }
        >
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <p className="card-sub">
          Tampil 4 sekaligus. Jika lebih dari 4, akan otomatis slide setiap 5 detik. Foto opsional.
        </p>
        {t.items.map((it, i) => (
          <div key={i} className="item-card">
            <span className="item-tag">#{i + 1}</span>
            <div className="flex justify-end mb-1">
              <button
                className="icon-btn"
                onClick={() => set({ items: t.items.filter((_, j) => j !== i) })}
              >
                <Trash2 size={14} />
              </button>
            </div>
            ...
            <div className="grid-2 mt-2">
              ...
              />
              <TextField
                label="Info (mis. Pernikahan · Nov 2024)"
                value={it.meta}
                onChange={(v) => {
                  const n = [...t.items];
                  n[i] = { ...n[i], meta: v };
        ...
        <button
          className="add-btn"
          onClick={() => set({ items: [...t.items, { text: "", name: "", meta: "" }] })}
        >
          <Plus size={14} /> Tambah
        </button>
      </div>
      <div className="card">
        ...
        {t.stats.map((st, i) => {
          const PreviewIcon =
            (Icons[st.icon as keyof typeof Icons] as React.ElementType) ?? Icons.Star;

          return (
            <div key={i} className="grid grid-cols-[80px_120px_1fr_auto] gap-2 mb-2 items-end">
              {/* Preview Icon */}
              <div>
  ...
  return (
    <>
      ...
      <div className="card">
          ...
          <div key={i} className="item-card">
            ...
            <div className="mt-2">
              {it.a.type === "text" ? (
                ...
                <TextArea
                  label="Jawaban (satu item per baris)"
                  value={it.a.content.join("\n")}
                  onChange={(v) => {
                    const n = [...f.items];
        ...
        <button
          className="add-btn"
          onClick={() => set({ items: [...f.items, { q: "", a: { type: "text", content: "" } }] })}
        >
          <Plus size={14} /> Tambah
        </button>
      </div>
...
function CTATab({ draft, update }: TabProps) {
  ...
  return (
    <div className="card">
      ...
      <div className="mt-3">
        <TitleEditor label="Judul" value={c.title} onChange={(v) => set({ title: v })} />
      </div>
      <div className="mt-3">
        <TextArea
          label="Deskripsi"
      ...
      <div className="mt-3">
        <TextField label="Teks Tombol" value={c.button} onChange={(v) => set({ button: v })} />
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="card">
        ...
        <div className="mt-3">
          <TextField label="Lokasi" value={k.location} onChange={(v) => set({ location: v })} />
        </div>
      </div>
    </>
  );
...
function AkunTab({ email }: { email: string | null }) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (newPassword !== confirmPassword) {
      setMessage({ kind: "err", text: "Konfirmasi password tidak cocok." });
      return;
    }
    if (newPassword.length < 8) {
      setMessage({ kind: "err", text: "Password baru minimal 8 karakter." });
      return;
    }
    setLoading(true);
    try {
      await changeAdminPassword({ data: { currentPassword, newPassword } });
      setMessage({ kind: "ok", text: "Password berhasil diubah." });
      setCurrentPassword("");
      setNewPassword("");
      setConfirmPassword("");
    } catch (err: any) {
      setMessage({ kind: "err", text: err?.message ?? "Gagal mengubah password." });
    } finally {
      setLoading(false);
    }
  };
